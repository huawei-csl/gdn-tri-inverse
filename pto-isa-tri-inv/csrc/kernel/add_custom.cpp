...
constexpr unsigned X_PONG =
    (X_PING + 0x8000 + 0x100);        // pong address of input x in UB buffer
constexpr unsigned Y_PING = 0x10000;  // ping address of input y in UB buffer
constexpr unsigned Y_PONG =
    (Y_PING + 0x8000 + 0x100);        // pong address of input y in UB buffer
constexpr unsigned Z_PING = 0x20000;  // ping address of output z in UB buffer
constexpr unsigned Z_PONG =
    (Z_PING + 0x8000 + 0x100);  // pong address of output z in UB buffer
...
AICORE void runTAdd(__gm__ T* z, __gm__ T* x, __gm__ T* y,
  ...
  static_assert(bTileRows * bTileCols * sizeof(T) <= MAX_TILE_SIZE,
                "UB buffer overflow.");
  ...
  using TileData =
      Tile<TileType::Vec, T, tileSRows, tileSCols, BLayout::RowMajor, -1, -1>;
  ...
  TileData xTiles[BUFFER_NUM] = {TileData(vRows, vCols),
                                 TileData(vRows, vCols)};
  TileData yTiles[BUFFER_NUM] = {TileData(vRows, vCols),
                                 TileData(vRows, vCols)};
  TileData zTiles[BUFFER_NUM] = {TileData(vRows, vCols),
                                 TileData(vRows, vCols)};
...
// kernel entry
extern "C" __global__ AICORE void add_custom_fp16(GM_ADDR x, GM_ADDR y,
                                                  GM_ADDR z,
                                                  uint32_t totalLength) {
  ...
  runTAdd<half, tileRows, tileCols>((__gm__ half*)z, (__gm__ half*)x,
                                    (__gm__ half*)y, totalLength);
}

extern "C" __global__ AICORE void add_custom_fp32(GM_ADDR x, GM_ADDR y,
                                                  GM_ADDR z,
                                                  uint32_t totalLength) {
  // Define the tile size
  constexpr unsigned tileRows = 20;
  constexpr unsigned tileCols = 2048;
  // main kernel, totalLength is dynamic input
  runTAdd<float, tileRows, tileCols>((__gm__ float*)z, (__gm__ float*)x,
