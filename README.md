...
Steps to run SGLang's `bench_one_batch` on docker:

```bash
# mounting sgl-kernel-npu is not necessary, but it is useful to test different inverse algorithms e2e
./docker/start_docker_910B2.sh --volume ~/sgl-kernel-npu/:/workspace/sgl-kernel-npu/
```

```bash
cd gdn-tri-inv-repo/
python docker/sglang-bench-one-batch.py Qwen/Qwen3.5-0.8B-Base
```

Add the `--update` flag to reinstall sgl-kernel-npu (needs the sgl-kernel-npu repo to be mounted when starting docker)
