...
def chunk_h_ker(
    B, H, L, DK, DV, C, BK=None, BV=None, dtype="float16", accum_dtype="float"
):
    if BK == None:
        BK = DK
    if BV == None:
    ...
    def main(
        ...
        workspace_3: T.Tensor(
            [B * H * bv_num, DK, BV], dtype
        ),  # need to be manually set to 0
        ...
        with T.Kernel(B * H * bv_num, is_npu=True) as (cid, vid):
            ...
            with T.Scope("V"):
                ...
                T.copy(
                    K[bz, by, vid * C // VEC_NUM, 0], k_ub_half
                )  # Preload K and g for the first chunk
                ...
                for i in T.serial(chunk_num):  # Calculate hidden state S chunk by chunk
                    ...
                    for i in range((C // VEC_NUM) // 4):
                        ...
                        T.tile.mul(
                            k_ub[i * 4 + 1, :], k_ub[i * 4 + 1, :], coeff_ub[i * 4 + 1]
                        )
                        T.tile.mul(
                            k_ub[i * 4 + 2, :], k_ub[i * 4 + 2, :], coeff_ub[i * 4 + 2]
                        )
                        T.tile.mul(
                            k_ub[i * 4 + 3, :], k_ub[i * 4 + 3, :], coeff_ub[i * 4 + 3]
                        )
                    ...
                    if i < chunk_num - 1:
                        T.copy(
                            K[bz, by, (i + 1) * C + vid * C // VEC_NUM, 0], k_ub_half
                        )  # Preload K and g for the next chunk
                        T.copy(
                            G[bz, by, (i + 1) * C], g_ub
                        )  # The g value of the whole chunk
                    ...
                    for i, j in T.Parallel(DK // VEC_NUM, BV):
                        s_ub[i, j] = (
                            s_ub[i, j] + kv_ub[i, j]
                        )  # S_next = S * exp(g_last) + \tilde K * New_V
                    ...
                    if i < chunk_num - 1:
                        ...
                        T.copy(
                            s_ub_half, S[bz, by, i + 1, vid * DK // VEC_NUM, bx * BV]
                        )  # Store state S at the end of this chunk
