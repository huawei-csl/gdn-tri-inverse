        ...
        with T.Kernel(B * (H // VEC_NUM) * chunk_num, is_npu=True) as (cid, vid):
            ...
            with T.Scope("V"):
                ...
                for i in range(2, C):
                    ...
                    T.tile.sub(
                        o_ub[i, :], o_ub[i, :], red_ub
                    )  # A_i - (A_i * (-B_{0...i-1})) = -B_i
...
def solve_tril_64_ker(B, H, L, dtype="float16", accum_dtype="float"):
    ...
    def main(
        ...
        with T.Kernel(B * H * chunk_num, is_npu=True) as (cid, vid):
            ...
            with T.Scope("V"):
                T.copy(
                    A[bz, by, bx * C + vid * F, vid * F], o_ub_half
                )  # Vector core 0 solves B_00, Vector core 1 solves B_11
                T.copy(I[0, 0], i_ub)
                T.set_flag("mte2", "v", 0)
                T.wait_flag("mte2", "v", 0)
                T.tile.fill(mul_ub, 0.0)
                ...
                if vid == 0:
                    ...
                    T.wait_flag("mte2", "v", 0)
                    T.tile.sub(
                        o_ub_half, zero_ub_half, o_ub_half
                    )  # O_10 = - O_11 * A_10 * O_00
                    T.set_flag("v", "mte3", 0)
                    T.wait_flag("v", "mte3", 0)
...
def solve_tril_128_ker(B, H, L, dtype="float16", accum_dtype="float"):
    ...
    def main(
        ...
        with T.Kernel(B * H * chunk_num, is_npu=True) as (cid, vid):
            ...
            tmp_ub = T.alloc_ub(
                [3 * DataType(accum_dtype).bits // 8 * F * F // VEC_NUM], "uint8"
            )
            ...
            with T.Scope("V"):
                ...
                T.wait_flag("v", "mte3", 0)
                T.copy(
                    zero_ub_half, O[bz, by, bx * C + (vid * 2) * F, (vid * 2 + 1) * F]
                )
                ...
                for ii in range(2):
                    T.copy(
                        A[bz, by, bx * C + (ii * 2 + vid) * F, (ii * 2 + vid) * F],
                        a_ub_half[ii, :, :],
                    ...
                    T.copy(
                        a_ub_half[ii, :, :],
                        O[bz, by, bx * C + (ii * 2 + vid) * F, (ii * 2 + vid) * F],
                    )
            ...
            with T.Scope("C"):
                ...
                for ii in range(2):
                    ...
                    T.copy(
                        mult_s_l0, O[bz, by, bx * C + (ii * 2 + 1) * F, (ii * 2) * F]
                    )
                    T.copy(
                        O[bz, by, bx * C + (ii * 2 + 1) * F, (ii * 2 + 1) * F], o22_s_l1
                    )
                    T.set_flag("fix", "mte2", 0)
                    T.wait_flag("fix", "mte2", 0)
                    T.copy(
                        O[bz, by, bx * C + (ii * 2 + 1) * F, (ii * 2) * F], mult_s_l1
                    )
                    T.gemm_v0(o22_s_l1, mult_s_l1, final_s_l0, init=True)
                    T.copy(
                        final_s_l0, O[bz, by, bx * C + (ii * 2 + 1) * F, (ii * 2) * F]
                    )
                    if ii == 0:
