        ...
        with T.Kernel(B * H * chunk_num, is_npu=True) as (cid, vid):
            ...
            with T.Scope("C"):
                for i in T.serial(bk_num):
                    ...
                    T.gemm_v0(
                        q_l1, k_l1, qk_l0, transpose_B=True, init=(i == 0)
                    )  # Q * K^T
                for i in T.serial(bk_num):
                    T.copy(Q[bz, by, bx * C, i * BK], q_l1)
                ...
                T.wait_cross_flag(1)
                T.copy(
                    workspace_3[cid, 0, 0], qk_l1
                )  # Gamma \odot Mask \odot (Q * K^T)
                for i in T.serial(bv_num):
                    T.copy(V[bz, by, bx * C, i * BV], v_l1)
                    T.gemm_v0(qk_l1, v_l1, qkv_l0, init=True)
                    T.copy(
                        qkv_l0, workspace_2[cid, 0, i * BV]
                    )  # Term 2 of the formula (intra-chunk)
            ...
            with T.Scope("V"):
                ...
                T.copy(
                    qk_ub_half, workspace_3[cid, vid * C // VEC_NUM, 0]
                )  # Gamma \odot Mask \odot (Q * K^T)
                ...
                for i in range((C // VEC_NUM) // 4):
                    ...
                    T.tile.mul(
                        qs_ub[i * 4 + 1, :], qs_ub[i * 4 + 1, :], g_v_ub[i * 4 + 1]
                    )
                    T.tile.mul(
                        qs_ub[i * 4 + 2, :], qs_ub[i * 4 + 2, :], g_v_ub[i * 4 + 2]
                    )
                    T.tile.mul(
                        qs_ub[i * 4 + 3, :], qs_ub[i * 4 + 3, :], g_v_ub[i * 4 + 3]
                    )
