...
export TILELANG_ASCEND_COMMIT="${TILELANG_ASCEND_COMMIT:-63bef06}" # v0.1.1.010-release
export CANN_VERSION="${CANN_VERSION:-9.0.0}" # Currently only 8.5.0 (with tilelang-ascend) and 9.0.0 (w/o tilelang-ascend) are supported
export DOCKER_IMAGE_TAG="${DOCKER_IMAGE_TAG:-gdn-tri-inverse:${CANN_VERSION}-25062026}"
export DOCKERFILE="${DOCKERFILE:-cann-${CANN_VERSION}.Dockerfile}"
